...
const unsigned long sendInterval = 180000; // 5 minutes in ms
  ...
  while (gpsSerial.available() > 0) {
    ...
    if (c == '\n') {
      ...
      if (nmea.startsWith("$GPRMC")) {
        //Serial.println("Gps function call");
        parseGPRMC(nmea);
        break;
      }
      nmea = "";
...
if (sensorValue >= 2970) 
     Pbattery = 24;
  else if (sensorValue >= 2850 && sensorValue < 2970) 
     Pbattery = 23;
  else if (sensorValue >= 2750 && sensorValue < 2850) 
     Pbattery = 22;
  else if (sensorValue >= 2695 && sensorValue < 2750) 
    Pbattery = 21;
  else if (sensorValue >= 2570 && sensorValue < 2695) 
     Pbattery = 20;
  else if (sensorValue >= 2400 && sensorValue < 2570) 
     Pbattery = 19;
  else if (sensorValue >= 2330 && sensorValue < 2400) 
   Pbattery = 18;
   
  if (sensorValue2 >= 2800) 
     Bbattery = 24;
 ...
 // Serial.println(analogRead(MAINS_VOLTAGE_SENSOR_PIN));
  if(analogRead(MAINS_VOLTAGE_SENSOR_PIN)<800)
    ...
    isMainsVoltagePresent=false;
     ...
     Serial.println("Pbattery : ");
     Serial.println(Pbattery);

     Serial.println("ssPbattery : ");
     Serial.println(sensorValue);

     Serial.println("ssBbattery : ");
...
void parseGPRMC(String s) {
  ...
  Serial.println("function inside");
